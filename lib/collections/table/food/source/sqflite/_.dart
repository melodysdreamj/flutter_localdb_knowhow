...
class FoodSqflite {
  Future<Food?> get(String docId) async {
    await _ready();
    List<Map> maps = await FoodDb.query('Food',
  ...
  Future<Food?> getByFoodName(String foodName) async {
    await _ready();
    List<Map> maps = await FoodDb.query('Food',
        // columns: [_columnId, s1, s2],//한정해야할필요가 있을때만 적용
        where: 'FoodName = ?',
  ...
  // Future<Food?> getByUpdateMillis(int value) async {
  //   await _ready();
  //   List<Map> maps = await FoodDb.query('Food',
  ...
  Future<List<Food>> getWhere(String where) async {
    await _ready();

    List<Map> maps = await FoodDb.query(
      'Food',
      where: where,
    );
    List<Food> result = [];
  ...
  Future<List<Food>> queryMoreThenUsingUpdateMillis(int time) async {
    await _ready();
  ...
  Future<int?> getRowCount() async {
    ...
    return Sqflite.firstIntValue(
        await FoodDb.rawQuery('SELECT COUNT(*) FROM Food'));
  }

  Future<void> upsert(Food newObject) async {
  ...
  insertBulk(List<Food> objects) async {
    Batch batch = FoodDb.batch();

    // for loop with index
    for (var i = 0; i < objects.length; i++) {
      var element = objects[i];
      batch.insert('Food', toMap(object: element));
    }

    await batch.commit(noResult: true);
  }

  Future<int> insert(Food object) async {
    await _ready();

    return await FoodDb.insert('Food', toMap(object: object));
  }

  Future<int> update(Food object, Food oldObject) async {
    await _ready();

    return await FoodDb.update('Food', toMap(object: object),
        where: 'DocId = ?', whereArgs: [oldObject.DocId]);
  }


  ///////////////////////////////////////
  Map<String, Object?> toMap({required Food object, String? inputId}) {
    ...
    map["Steps"] = jsonEncode(object.Steps);
    ...
    map["Ingredients"] = jsonEncode(object.Ingredients.map((model) => model.toString()).toList());
  ...
  Food fromMap(Map<dynamic, dynamic> map) {
    Food object = Food();
    ...
    object.Steps = jsonDecode(map["Steps"] ?? "[]");
    ...
    object.Ingredients = (jsonDecode(map["Ingredients"] ?? "[]") as List).map((item) => Ingredient.fromString(item)).toList();
    // object.J001 = (jsonDecode(map["J001"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J002 = (jsonDecode(map["J002"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J003 = (jsonDecode(map["J003"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J004 = (jsonDecode(map["J004"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J005 = (jsonDecode(map["J005"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J006 = (jsonDecode(map["J006"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J007 = (jsonDecode(map["J007"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J008 = (jsonDecode(map["J008"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J009 = (jsonDecode(map["J009"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J010 = (jsonDecode(map["J010"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J011 = (jsonDecode(map["J011"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J012 = (jsonDecode(map["J012"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J013 = (jsonDecode(map["J013"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J014 = (jsonDecode(map["J014"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J015 = (jsonDecode(map["J015"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J016 = (jsonDecode(map["J016"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J017 = (jsonDecode(map["J017"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J018 = (jsonDecode(map["J018"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J019 = (jsonDecode(map["J019"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
    // object.J020 = (jsonDecode(map["J020"] ?? "[]") as List).map((item) => Food2.fromString(item)).toList();
  ...
  Future _open() async {
    await _prepare();
    await FoodDb.transaction((txn) async {
      // Ok
      await txn.execute("create table if not exists Food ("
    ...
    await createIndexing(FoodDb);
  }

  _handleColumnChanged() async {
    var _ = ManagingSqfliteTableFood().get();

    var columns = paramListFood.map((e) => e[0].toString()).toList();
    if (_.UpdateMillis == 0) {
      // 만약 한번도 작동한 적이 없다면 바로 컬럼에 넣어줍니다.
      _.Version = 1;
      _.UpdateMillis = DateTime.now().millisecondsSinceEpoch;
      _.Columns = columns;
      ManagingSqfliteTableFood().upsert(_);
    }
    ...
    for (var item in needCreateParams) {
      var param = paramListFood.firstWhere((element) => element[0] == item);
  ...
  _addColumns(String newColumnName, String columnType) async {
    ...
    await FoodDb.execute(
        "ALTER TABLE Food ADD COLUMN $newColumnName $columnType DEFAULT $defaultValue");
  }
  ...
  _removeColumns(List<dynamic> columnsToDelete) async {
    ...
    // 임시 테이블 이름 생성
    String tempTableName = 'temp_Food';

    // 기존 테이블의 모든 컬럼 정보 가져오기
    List<Map> columns =
        await FoodDb.rawQuery('PRAGMA table_info(Food)');
    ...
    ''');
      ...
      // 기존 테이블 삭제
      await txn.execute('DROP TABLE Food');

      // 임시 테이블의 이름을 원래 테이블 이름으로 변경
      await txn.execute('ALTER TABLE $tempTableName RENAME TO Food');
    });
  }
  ...
  _prepare() async {
    ...
    FoodDb = await openDatabase(path,
        version: ManagingSqfliteTableFood().get().Version);
    _isDbOpened = true;
  }
  ...
  createIndexing(Database myDatabase) async {
    ...
    if (box.read("FoodSqfliteIndex") != null) {
      return;
    }
    box.write("FoodSqfliteIndex", true);

    /// 예시
    // await myDatabase.execute("create index UpdateMillisindex on Food (UpdateMillis)"); // 단일 인덱싱
    // await myDatabase.execute("create index UpdateMillisindex on Food (UpdateMillis DESC)"); // 단일 인덱싱 내림차순
    // await myDatabase.execute("create index FoodNameUpdateMillisindex on Food (FoodName ASC, UpdateMillis DESC)"); // 다중 컬럼 인덱싱
